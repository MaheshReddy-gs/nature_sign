...
//micro
 { id: "police", type: "micro", x: 9.4, y: 33.3, label: "Superintendent <br/>Of Police Office" ,labelPosition: "top" },
  ...
  { id: "hospital", type: "micro", x: 18.6, y: 54.4,labelPosition: "left", label: "Sri Madhusudan Sai<br/> Institute Of Medical Sciencesi<br/> And Research" },
  ...
  { id: "school", type: "micro", x: 67.4, y: 85.3, label: "Acharya Rakumjee Ashram And School" },
  ...
  // Change these titles to swap out which pins appear in the strip
  const QUICK_LINK_TITLES = [
    "Kempegowda International Airport",
    "Nandi Hills",
    "KMF Mega Dairy",
    "Krishna Convention Center",
    "SAP Labs India",
    "Foxconn",
    "Prestige Golfshire Club",
    "Akash Super Speciality Hospital",
    "Wipro Limited",
    "Manyata Embassy Business Park",
  ];

  const quickLinks = QUICK_LINK_TITLES
    .map(title => resolvedPins.find(p => p.title === title))
    .filter(Boolean);
  ...
  // ── Navigate map to a pin smoothly ──
  const navigateToPin = (pin) => {
    const isMobile = window.matchMedia("(max-width: 767px)").matches;
    const container = isMobile ? mapRef.current : desktopMapRef.current;
    if (!container) return;

    const { width, height } = container.getBoundingClientRect();
    const TARGET_ZOOM = 2.4;

    // Account for legacy pin layer's vertical offset
    const actualY = pin.legacy
      ? pinTopOffset + (pin.y / 100) * (100 - pinTopOffset)
      : pin.y;
    ...
    if (!isMobile) {
      // Desktop — transformOrigin: "center center"
      ...
      // Mobile — transformOrigin: "center top"
      ...
      const imgW = imgRef?.naturalWidth && imgRef?.naturalHeight
        ? (imgRef.naturalWidth / imgRef.naturalHeight) * imgH
        : width;
      ...
      ([entry]) => {
        ...
        if (enteredFromTop && currentRatio > 0) {
          ...
          if (container) {
            ...
            setActivePin(resolvedPins[0]);
            setIsStripMinimised(false);
            setQuickActivePin(null);
          }
        }
  ...
  const renderPins = (legacyOnly) =>
    resolvedPins
      ...
      .map(pin => {
        ...
        // ── Is this pin the quick-nav highlighted pin? ──
        const isQuickActive = !!(
          quickActivePin &&
          quickActivePin.title === pin.title &&
          quickActivePin.x === pin.x &&
          quickActivePin.y === pin.y
        );
        ...
        return (
          <div
            ...
            style={{
              ...
              opacity: 1,
              pointerEvents: isVisible ? "auto" : "none",
              zIndex: isQuickActive ? 15 : activePin.id === pin.id ? 10 : 5,
            }}
          >
            <motion.div
              {...animateProps}
            ...
            >
              {/* Quick-nav highlight ring — pulsing orange halo */}
              {isQuickActive && (
                <motion.div
              ...
              <div
                className="relative z-10 flex items-center justify-center rounded-full transition-all duration-300 ease-in-out"
                style={{
                  width: `${dotSize}px`,
                  height: `${dotSize}px`,
                  background: isQuickActive ? "#f97316" : "white",
                  boxShadow: isQuickActive ? "0 0 0 3px rgba(249,115,22,0.4)" : undefined,
                  opacity: isVisible ? 1 : 0,
                }}
              >
                {customIcons[pin.id] ? (
                  <img
                    ...
                    style={{
                      width: iconSize,
                      height: iconSize,
                      filter: isQuickActive ? "brightness(0) invert(1)" : undefined,
                    }}
                    draggable={false}
                  />
                ) : (
      ...
      >
        {quickLinks.map((pin, i) => {
          const isActive =
            quickActivePin?.title === pin.title &&
            quickActivePin?.x === pin.x;
          // Strip HTML line-break tags to get a clean plain label
        ...
        <div className="max-w-7xl mx-auto px6">
          ...
          <FloatUpText className="hidden md:flex w-full justify-center overflow-hidden mt-6 md:mt-10">
            <div className="w-full max-w-7xl">
                ...
                >
                  ...
                  <div
                    style={{
                      ...
                      transformOrigin: "center center",
                      // ── Smooth transition for quick-nav, snappy for regular pan/zoom ──
              ...
              {/* ── DESKTOP QUICK LINKS STRIP (below map+details grid) ── */}
              <QuickLinksStrip />
          ...
          <div className="md:hidden mt-6">
              ...
              >
                <div
                  style={{
                    ...
                    // ── Smooth transition for quick-nav ──
                    transition: isPanning
                      ? "none"
                      : isNavigating
                        ? "transform 0.65s cubic-bezier(0.4, 0, 0.2, 1)"
            ...
            {/* ── MOBILE QUICK LINKS STRIP (below the map card) ── */}
            <div className="px-1">
              <QuickLinksStrip />
            </div>
          </div>
